<template>
  <div>
    <tab :line-width='line_width' :active-color='active_color' v-model="index">
      <tab-item class="vux-center" :selected="selected.code === item.code" v-for="(item, index) in list"
                @click.native="toggleDown(item)" :key="index">{{item.name}}&nbsp;
        <div class="toggleDown" :style="item.show?'transform:rotate(180deg)':'transform:rotate(0deg)'"></div>
      </tab-item>
    </tab>

    <div class="hotList"
         :style="selected.code === 'hot' && selected.show ?'height:140px;overflow:auto':'height:0px; overflow:hidden'">
      <swiper height="140px" :show-dots="hotListTran&&hotListTran.length>1?true:false" dots-position="center"
              dots-class="dotsClass">
        <swiper-item v-for="(item, index) in hotListTran" :key="index">
            <span v-for="(itemSub, index) in item" style="float: left;width: 33.33%;">
              <x-button class="linkButton" style="width: 95%;" mini
                        @click.native="linkHandler(itemSub)">{{itemSub.gameName}}</x-button>
            </span>
        </swiper-item>
      </swiper>
    ...
    <div class="likeList"
      ...
      <swiper v-if="likeListTran&&likeListTran.length>0" height="140px"
              :show-dots="likeListTran&&likeListTran.length>1?true:false" dots-position="center" dots-class="dotsClass">
        <swiper-item v-for="(item, index) in likeListTran" :key="index">
            <span v-for="(itemSub, index) in item" style="float: left;width: 33.33%;position: relative"
                  v-if="item.length>0" :class="showLikeDelete === true?'headShake':''">
              <x-button class="linkButton" style="width: 95%;" mini
                        @click.native="linkHandler(itemSub)">{{itemSub.gameName}}
              </x-button>
              <transition name="slide-fade">
                <span @click.stop="deleteLike(itemSub)" v-show="showLikeDelete === true"
                      class="delete_sub_btn headShake">
                  <x-icon type="close-circled" style="fill:#d23748" size="16"></x-icon>
                </span>
              </transition>
            </span>
          <span class="delete" @click="showDeleteLikeBtn">编辑 <span class="delete_btn"></span></span>
        </swiper-item>
    ...
    <div class="hotList"
      ...
      <draw-history v-if="selected.code === 'history' && selected.show" :height="'140px'" :options="drawHistoryOptions"></draw-history>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      linkHandler (item) {
        this.$router.push({path: `/game/${item.typeCode}/${item.gameTypeId}/${item.gameId}/${item.gameCode}/${item.gameName}`})
      },
      showDeleteLikeBtn () {
