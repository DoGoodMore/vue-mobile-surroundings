...
<script>
  ...
  import { getPerIssue, isNumber, parseDrawResult } from 'common/js/util'
  export default {
    data () {
      const loading = [
    ...
    methods: {
      ...
      _initTimer ({lotteryGroupId, lotteryId, stopSell}) {
        this.groupId = lotteryGroupId
        this.gameId = lotteryId
        this.reset()
        this._timerClear()
      ...
      _fetchDrawInfo (stopSell) {
        const _this = this
        getLotterysInfo({lotteryGroupId: _this.groupId, gameId: _this.gameId}).then(response => {
          ...
          if (currentStatus === ERR_OK && currentData.length) {
            for (let i = 0; i < currentData.length; i++) {
              ...
              if (Number(lotteryGameId) === Number(_this.gameId)) {
